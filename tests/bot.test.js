...
test('buildSettingsMainKeyboard returns a grouped layout with section buttons', () => {
  ...
  assert.deepEqual(keyboard[2].map((button) => button.text), ['⚙️ Команды', '🤖 Медиа ИИ']);
  assert.deepEqual(keyboard[3].map((button) => button.text), ['💬 Первый комментарий', '🚨 @admin', '🚫 Скрытые пользователи']);
  ...
  assert.equal(keyboard[2][0].callback_data, 'settings:section:commands:42');
  assert.equal(keyboard[2][1].callback_data, 'settings:section:media_ai:42');
  assert.equal(keyboard[3][0].callback_data, 'settings:open_menu:42');
  assert.equal(keyboard[3][1].callback_data, 'settings:section:admin:42');
  assert.equal(keyboard[3][2].callback_data, 'settings:section:anonymous:42');
});
